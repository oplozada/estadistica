...
def Kendall (matriz_aj, alpha=0.05):
    ...
    None
        Imprime en pantalla:
        - El valor del coeficiente W de Kendall.
        - El estadístico chi-cuadrado calculado.
        - El valor crítico de chi-cuadrado.
        - El valor p.
        - Conclusión sobre la hipótesis de concordancia.
    """
...
matriz_ajustada = []
for fila in data:
    fila_ajustada = coef_ajustado(fila)  #  rangos + cof de corrección
    matriz_ajustada.append(fila_ajustada )
...
print(f"Chi-cuadrado crítico (α = 0.01): {resultados['chi_critico']:.4f}")
...
if resultados['concordancia']:
    ...
    print("[x] No se puede afirmar concordancia significativa (no se rechaza H₀)")
